...
function generateMessage(order, product) {
  return `
📋 BÁO GIÁ IN ẤN

Khách hàng: ${order.ten_khach}
Sản phẩm: ${product.ten_sp}
Số lượng: ${formatNumber(order.so_luong)}
...
app.post("/quote", (req, res) => {
  try {
    ...
    const message = generateMessage(result, product);
...
app.get("/dashboard", (req, res) => {
  ...
  const donHangSheet = wb.Sheets["DON_HANG"];
  const data = donHangSheet ? XLSX.utils.sheet_to_json(donHangSheet) : [];
  ...
  data.forEach(d => {
    ...
    // khách hàng
    if (!khachMap[d.ten_khach]) {
      khachMap[d.ten_khach] = 0;
    }
    khachMap[d.ten_khach] += d.tong || 0;

    // nhà cung cấp
    if (!nccMap[d.nha_cung_cap]) {
      nccMap[d.nha_cung_cap] = 0;
    }
